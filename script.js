...
function checkPasswords() {
    ...
    if (!phoneNumber.value.match(/^(\+[(]?[0-9]{1,3}[)]?[-\s\.]?)?[0-9]{3}[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{3,4}$/)) {
    ...
    if (!password.value.match(/[#?!@$ %^&*-]/)) {
        setErrorFor(password, '• password needs to contain special special symbols #?!@$ %^&*-\n')
        result = false;
    }
    if (!password.value.match(/[a-z]/)) {
        setErrorFor(password, '• password needs to contain at least one lower case letter\n')
        result = false;
    }
    if (!password.value.match(/[A-Z]/)) {
        setErrorFor(password, '• password needs to contain at least one upper case letter\n')
        result = false;
    }
...
function submitForm() {
    const accountCreatedScreen = document.querySelector('.account-created-screen');
